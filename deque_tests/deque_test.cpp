...
namespace dequetest
{
	...
	public:
		TEST_METHOD(resize) {
			n = 8;
			my_deque d = my_deque(n);
			for (int i = 0; i < 6; i++) {
				d.push_back(i);
			}
			//resize when elements are >= size/2
			Assert::AreEqual((int)d.get_size(), 16);

			for (int i = 0; i < 6; i++) {
				d.pop_back();
			}

			Assert::AreEqual((int)d.get_size(), 8);
	...
	public:
		...
		TEST_METHOD(less_than)
		{
			...
			auto func = [] {
				...
				my_deque d2(2);
				my_deque_iterator di(d.get_mem() + d.get_rIndex(), &d);
				my_deque_iterator d2i(d2.get_mem() + d2.get_rIndex(), &d2);
				di < d2i; 
			};
			Assert::ExpectException<std::exception>(func);
		}
